...
<body class="text-gray-800 min-h-screen flex flex-col">
  ...
  <div class="max-w-xl mx-auto p-4 mt-6 w-full flex-grow">
    ...
    <div class="glass p-6 rounded-3xl shadow-xl mb-6 text-center">
      <input type="text" id="em" class="w-full p-4 rounded-2xl border-2 border-blue-100 text-center bg-white text-blue-700 font-bold mb-4" readonly>
      <button onclick="sl()" class="w-full bg-blue-600 text-white font-bold py-4 rounded-2xl mb-3 shadow-lg">SALIN EMAIL 📋</button>
    ...
    <div class="glass rounded-3xl p-6 text-center shadow-lg border-t-4 border-t-blue-500">
      <p class="text-xs text-gray-600 mb-4">Dibuat oleh <b>Habi</b>. Hubungi:</p>
      <a href="https://wa.me/6285119821813" target="_blank" class="bg-green-500 text-white font-bold py-2 px-6 rounded-full text-sm">Chat WA Habi</a>
    </div>
  </div>
  ...
  <script>
    ...
    function uw() {
      const n = new Date(); let h = n.getHours(); const m = String(n.getMinutes()).padStart(2,'0'); const s = String(now.getSeconds()).padStart(2,'0');
    ...
    async function mp() {
      try {
        ...
        if (d.length > lc) {
          if (lc !== 0) { sd.play(); if(Notification.permission==='granted') new Notification("Pesan Baru!"); }
          lc = d.length;
          document.getElementById('ib').innerHTML = d.map(p => {
            return '<div class="text-left border-b border-gray-100 pb-4">' +
                   ...
                   '<div class="bg-gray-800 p-3 rounded-xl text-[11px] text-gray-200">'+p.body+'</div>' +
                   '</div>';
          }).join('');
        }
      } catch (e) {}
    }
    window.onload = () => { gr(); setInterval(() => { const n = new Date(); let h = n.getHours(); const m = String(n.getMinutes()).padStart(2,'0'); const s = String(n.getSeconds()).padStart(2,'0'); const ap = h >= 12 ? 'PM' : 'AM'; h = h % 12 || 12; document.getElementById('jam').innerText = h + ':' + m + ':' + s + ' ' + ap; }, 1000); setInterval(mp, 1000); };
  </script>
</body>
</html>`;
